...
export default function RenewAddNewApplication() {
  const [active, setActive] = useState(null);
  const [reqGetOneApp, resGetOneApp] = useGetAppIdApplicationMutation();
  ...
  const { paymentData } = applications;

  useEffect(() => {
    reqGetOneApp({ appId, token });
    window.localStorage.setItem("accessToken", JSON.stringify(token));
  }, [appId, reqGetOneApp, token]);

  useEffect(() => {
    if (resGetOneApp.isSuccess) {
      setLoading(false);

      window.localStorage.setItem("payment", paymentData?.payment);
      window.localStorage.setItem("email", paymentData?.email);
      window.localStorage.setItem("appId", paymentData?._id);
      dispatch(updatePaymentData(resGetOneApp.data));
    }
    if (resGetOneApp.isLoading) {
      setLoading(true);
    }
  }, [resGetOneApp, dispatch, email, paymentData]);
  ...
  const handlePaypalAction = () => {
    const _payload = {
      ...
      desc: `payment for gatepass ,application type ${
        choiceType[paymentData?.applicationType]
      } with vehicle ${vhTpe[paymentData?.typeOfVehicle]} plate number is ${
  ...
  const handleSubmit = () => {
    switch (active) {
      ...
      case "gcashngkalukuhan":
        return navigate(
          `/gcashngkalukuhan?email=${email}&token=${token}&type=${paymentData?.applicationType}&vehicle=${paymentData?.typeOfVehicle}`
        );
      default:
        return;
    }
