...
import { dLicenseSchema } from "../../../schema/file";
...
const DLicense = ({
  ...
  const {
    ...
    resolver: yupResolver(dLicenseSchema),
  });
  ...
  const fileSingleRemove = () => {
    dispatch(deleteFiles({ dLicense: null }));
  };
  ...
  const onSubmit = (data) => {
    if (singleFile) {
      reqAddRequirements({
        email,
        requirements: upload.dLicense.name,
        requirementExp: data.dLicenseExp,
        type,
      });
    } else {
      ...
      <Box>
        {upload.dLicense !== null && (
          ...
          >
            ...
            >
              <img
                src={ImageConfig[upload?.dLicense?.type]}
                alt="upload"
                style={{
                  height: "2.4rem",
                  objectFit: "contain",
