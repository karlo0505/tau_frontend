...
import { mpPermitSchema } from "../../../schema/file";
...
const MpPermit = ({
  ...
  const {
    ...
    resolver: yupResolver(mpPermitSchema),
  });
  ...
  const fileSingleRemove = () => {
    dispatch(deleteFiles({ mpPermit: null }));
  };
  ...
  const onSubmit = (data) => {
    if (singleFile) {
      reqAddRequirements({
        email,
        requirements: upload.mpPermit.name,
        requirementExp: data.mpPermitExp,
        type,
      });
    } else {
  ...
  return (
    <Box sx={{ margin: "20px 0px" }}>
      <FormLabel>Mayor's permit expiration date</FormLabel>
      <TextField
        {...register("mpPermitExp")}
        type="date"
        InputLabelProps={{
          shrink: true,
        }}
        ...
        {upload.mpPermit !== null && (
          ...
          >
            <Box display="flex" sx={{ width: "100%" }}>
              <img
                src={ImageConfig[upload?.mpPermit?.type]}
                alt="upload"
                style={{
                  height: "2.4rem",
                  objectFit: "contain",
