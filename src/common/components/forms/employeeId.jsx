...
import { employeeIdSchema } from "../../../schema/file";
...
const EmployeeId = ({
  ...
  const fileSingleRemove = () => {
    dispatch(deleteFiles({ employeeId: null }));
  };
  ...
  const onSubmit = (data) => {
    if (singleFile) {
      reqAddRequirements({
        email,
        requirements: upload.employeeId.name,
        requirementExp: data.employeeIdExp,
        type,
      });
    } else {
  ...
  return (
    <Box sx={{ margin: "20px 0px" }}>
      <FormLabel>Employee ID expiration date</FormLabel>
      <TextField
        {...register("employeeIdExp")}
        type="date"
        InputLabelProps={{
          shrink: true,
        }}
        ...
        {upload.employeeId !== null && (
          ...
          >
            <Box display="flex" sx={{ width: "100%" }}>
              <img
                src={ImageConfig[upload?.employeeId?.type]}
                alt="upload"
                style={{
                  height: "2.4rem",
                  objectFit: "contain",
