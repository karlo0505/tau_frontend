...
import { studentIdSchema } from "../../../schema/file";
...
const StudentId = ({
  ...
  const fileSingleRemove = () => {
    dispatch(deleteFiles({ studentId: null }));
  };
  ...
  const onSubmit = (data) => {
    if (singleFile) {
      reqAddRequirements({
        email,
        requirements: upload.studentId.name,
        requirementExp: data.studentIdExp,
        type,
      });
    } else {
  ...
  return (
    <Box sx={{ margin: "20px 0px" }}>
      <FormLabel>Student ID Expiration Date</FormLabel>
      <TextField
        {...register("studentIdExp")}
        type="date"
        InputLabelProps={{
          shrink: true,
        }}
      ...
      <Box>
        {upload.studentId !== null && (
          ...
          >
            ...
            >
              <img
                src={ImageConfig[upload?.studentId?.type]}
                alt="upload"
                style={{
                  height: "2.4rem",
                  objectFit: "contain",
