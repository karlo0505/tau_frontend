...
import { orRecieptSchema } from "../../../schema/file";
...
const CRReciept = ({
  ...
  const {
    ...
    resolver: yupResolver(orRecieptSchema),
  });
  ...
  const fileSingleRemove = () => {
    dispatch(deleteFiles({ orReciept: null }));
  };
  ...
  const onSubmit = (data) => {
    if (singleFile) {
      reqAddRequirements({
        email,
        requirements: upload.orReciept.name,
        requirementExp: data.orRecieptExp,
        type,
      });
    } else {
  ...
  return (
    <Box sx={{ margin: "20px 0px" }}>
      <FormLabel>Vehicle official reciept Expiration Date</FormLabel>
      <TextField
        {...register("orRecieptExp")}
        type="date"
        InputLabelProps={{
          shrink: true,
        }}
        ...
        {upload.orReciept !== null && (
          ...
          >
            <Box display="flex" sx={{ width: "100%" }}>
              <img
                src={ImageConfig[upload?.orReciept?.type]}
                alt="upload"
                style={{
                  height: "2.4rem",
                  objectFit: "contain",
